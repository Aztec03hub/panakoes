...
if git -C "$WORKTREE" rev-parse --verify "$BASE_REF" >/dev/null 2>&1; then
    # Read commit subjects since base. Skip merge / revert commits: those
    # subjects are generated by git or GitHub (auto-update-branch creates
    # "Merge branch 'main' into ..."; git revert prepends "Revert ") and
    # do not follow Conventional Commits by design. Flagging them as
    # non-conforming was a recurring false-positive seen 2026-05-19.
    while IFS=$'\t' read -r sha subject parents; do
        ...
        # Skip subjects that start with the git/GH boilerplate prefixes
        case "$subject" in
    ...
    done < <(git -C "$WORKTREE" log --format='%h%x09%s%x09%P' "$BASE_REF..HEAD" 2>/dev/null)
fi
