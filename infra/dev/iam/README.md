...
| `auth`               | `ecs-tasks.amazonaws.com`   | audit-log Put, DDB Query/GetItem on `panakoes-dev-subscriptions` (read-only; for the sign-in plan-claim lookup, forward-referenced; the billing slice provisions the table). Postgres access is via VPC + security groups, not IAM. Secrets Manager (database-url, jwt-signing) is loaded at task startup by the execution role, not the task role. |
