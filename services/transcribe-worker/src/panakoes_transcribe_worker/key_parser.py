...
The Ingestion API writes objects under
`audio/{user_id}/{ingestion_id}/{filename}` (see
`services/ingestion-api/.../storage/s3.py::build_object_key`).
This file mirrors `services/event-router/.../key_parser.py`; the two
copies are intentional. Coupling two Lambda services through a shared
`panakoes-storage` library is more cost than the four-line helper is
worth at this stage. If the key shape ever changes, both copies move
in lockstep (search for `_PREFIX = "audio/"`).
...
def parse_object_key(raw_key: str) -> ParsedKey | None:
    ...
    raising. We accept exactly three `/`-separated segments after the
