"""Integration tests for `POST /api/v1/admin/tenants/{id}/force-billing-recompute`."""
...
class FakeEventBridgePublisher:
    """In-memory publisher; can be configured to raise on publish."""
...
async def test_force_billing_recompute_happy_path(
    client_admin: AsyncClient, publisher: FakeEventBridgePublisher
) -> None:
    response = await client_admin.post(
        "/api/v1/admin/tenants/tenant_42/force-billing-recompute",
        json=_body("op-1", "RECOMPUTE BILLING tenant_42", "month-end correction"),
    )
    assert response.status_code == 200, response.text
    body = response.json()
    assert body["status"] == "succeeded"
    assert body["result"]["tenant_id"] == "tenant_42"
    assert body["result"]["queued_reason"] == "month-end correction"
    assert body["result"]["eventbridge_event_id"] == "evt-0001"
    assert len(publisher.calls) == 1
    assert publisher.calls[0]["detail_type"] == "BillingRecomputeRequested"


@pytest.mark.integration
async def test_force_billing_recompute_wrong_confirmation_returns_400(
    client_admin: AsyncClient,
) -> None:
    response = await client_admin.post(
        "/api/v1/admin/tenants/tenant_42/force-billing-recompute",
        json=_body("op-2", "RECOMPUTE BILLING wrong", "x"),
    )
    assert response.status_code == 400


@pytest.mark.integration
async def test_force_billing_recompute_publisher_failure_returns_failed_envelope(
    aws_resources: dict[str, Any],
) -> None:
    publisher = FakeEventBridgePublisher(raise_always=True)
    app.dependency_overrides[require_admin_with_step_up] = _admin_with_step_up
    app.dependency_overrides[get_audit_table] = lambda: aws_resources["audit"]
    app.dependency_overrides[get_lifecycle_state] = lambda: aws_resources[
        "lifecycle_state"
    ...
    try:
        ...
        async with AsyncClient(transport=transport, base_url="http://testserver") as c:
            response = await c.post(
                "/api/v1/admin/tenants/tenant_42/force-billing-recompute",
                json=_body("op-3", "RECOMPUTE BILLING tenant_42", "x"),
            )
        assert response.status_code == 200
        body = response.json()
        assert body["status"] == "failed"
        assert "eventbridge" in body["error_message"].lower()
    finally:
        app.dependency_overrides.clear()


@pytest.mark.integration
async def test_force_billing_recompute_idempotent_replay(
    client_admin: AsyncClient,
    aws_resources: dict[str, Any],
    publisher: FakeEventBridgePublisher,
) -> None:
    body = _body("op-4", "RECOMPUTE BILLING tenant_42", "first")
    first = await client_admin.post(
        "/api/v1/admin/tenants/tenant_42/force-billing-recompute", json=body
    )
    ...
    second = await client_admin.post(
        "/api/v1/admin/tenants/tenant_42/force-billing-recompute",
        json=_body("op-4", "RECOMPUTE BILLING tenant_42", "second"),
    )
...
@pytest.mark.integration
async def test_force_billing_recompute_unauthenticated_returns_401(
    aws_resources: dict[str, Any], publisher: FakeEventBridgePublisher
) -> None:
    app.dependency_overrides.clear()
    pub: EventBridgePublisher = publisher
    ...
    try:
        ...
        async with AsyncClient(transport=transport, base_url="http://testserver") as c:
            response = await c.post(
                "/api/v1/admin/tenants/tenant_42/force-billing-recompute",
                json=_body("op-5", "RECOMPUTE BILLING tenant_42", "x"),
            )
