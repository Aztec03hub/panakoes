...
async def test_health_endpoint_returns_ok() -> None:
    ...
    This test deliberately does NOT depend on the moto / dynamodb
    fixtures; `/health` must be reachable even when AWS is unavailable.
    """
    ...
    body = response.json()
    assert body == {"status": "ok", "service": "session-manager"}
