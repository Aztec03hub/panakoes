...
@pytest.mark.integration
async def test_list_notifications_returns_only_callers_records(
    async_client: AsyncClient,
    make_token: Any,
) -> None:
...
@pytest.mark.integration
async def test_list_notifications_returns_records_for_owner(
    async_client: AsyncClient,
    auth_headers: dict[str, str],
) -> None:
...
@pytest.mark.integration
async def test_list_notifications_paginates_with_cursor(
    async_client: AsyncClient,
    auth_headers: dict[str, str],
) -> None:
    ...
    first = await async_client.get("/notifications?limit=2", headers=auth_headers)
    ...
    second = await async_client.get(
        f"/notifications?limit=2&cursor={first_body['next_cursor']}",
        headers=auth_headers,
    )
    assert second.status_code == 200
    second_body = second.json()
    assert len(second_body["items"]) == 2
    first_ids = {item["notification_id"] for item in first_body["items"]}
    second_ids = {item["notification_id"] for item in second_body["items"]}
    assert first_ids.isdisjoint(second_ids)


@pytest.mark.integration
async def test_list_notifications_rejects_oversized_limit(
    async_client: AsyncClient,
    auth_headers: dict[str, str],
) -> None:
