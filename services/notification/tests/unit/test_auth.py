"""JWT validation edge cases for `panakoes_notification.auth`.
...
from panakoes_notification.auth import (
    AuthenticatedUser,
...
@pytest.mark.unit
def test_verify_jwt_accepts_service_actor_type(
    test_settings: Settings,
    make_token: Any,
) -> None:
    """A service-to-service token (actor_type=service) is accepted."""
    token = make_token(
        sub="svc_summarization",
        email="summarization@panakoes",
        jti="svc_session",
        actor_type="service",
    )
    user = verify_jwt(token, test_settings)
    assert user.actor_type == "service"
    assert user.user_id == "svc_summarization"
...
def test_verify_jwt_rejects_unknown_actor_type(
    ...
    """An unknown actor_type value is rejected."""
...
@pytest.mark.unit
def test_verify_jwt_rejects_non_string_sub_claim(test_settings: Settings) -> None:
    """Numeric `sub` is rejected."""
    payload = {
        "sub": 12345,
        "email": "a@b.com",
        "jti": "j",
...
@pytest.mark.unit
def test_settings_helper_is_cached() -> None:
    """The `_settings()` helper caches its result via lru_cache."""
    first = _settings()
    second = _settings()
