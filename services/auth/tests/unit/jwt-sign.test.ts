...
describe("signJwt", () => {
  it("mints a token with the spec'd claims", async () => {
    const { token, expiresAt } = await signJwt(
      { sub: "user-uuid", email: "user@example.com", jti: "session-uuid" },
      config,
    );
  ...
  it("uses HS256 in the JWT header", async () => {
    const { token } = await signJwt({ sub: "u", email: "e@e.com", jti: "j" }, config);
  ...
  it("respects the configured expiry window", async () => {
    ...
    const { expiresAt } = await signJwt({ sub: "u", email: "e@e.com", jti: "j" }, shortConfig);
  ...
  it("rejects tampered payloads on verify (signature mismatch)", async () => {
    ...
    const tamperedPayload = Buffer.from(
      JSON.stringify({ sub: "attacker", email: "x@x.com", jti: "stolen" }),
    ).toString("base64url");
