...
Every model carries `frozen=True`, so you cannot accidentally mutate a record in flight. To produce a "modified" copy, use `record.model_copy(update={...})`.
...
Coverage gate: `--cov-fail-under=95` enforced in `pyproject.toml`.
