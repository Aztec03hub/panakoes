"""JWT validation edge cases for `panakoes_billing.auth`.

Auth code paths must hit 100% coverage per ADR-018, so every error
branch is exercised here. Mirrors the ingestion-api auth test surface
since the contract and implementation match.
"""
...
@pytest.mark.unit
def test_settings_dependency_is_cached() -> None:
    """`_settings()` caches its return value across calls.

    The `lru_cache` is the reason we don't re-read env on every request;
    a regression that drops the cache decorator would silently double
    Settings() construction cost. The cache survives across calls within
    a single process; tests reset it via `_settings.cache_clear()` so
    env-var fixtures stay deterministic.
    """
