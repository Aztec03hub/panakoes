"""FastAPI entrypoint for the Panakoes Query API."""
...
from panakoes_query_api.routes import health, ingestions, sessions, summaries
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    Query API reads from DynamoDB via boto3 (instrumented) and serves
    HTTP via FastAPI (instrumented). It does not use `httpx` for
    outbound calls, so we skip that hook.
    """
    otel_configure(
