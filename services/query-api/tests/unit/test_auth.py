"""JWT validation edge cases for `panakoes_query_api.auth`.
...
from panakoes_query_api.auth import (
    AuthenticatedUser,
...
@pytest.mark.unit
def test_settings_helper_returns_settings_instance() -> None:
    """`_settings()` returns a usable cached `Settings`."""
    _settings.cache_clear()
    settings = _settings()
    assert isinstance(settings, Settings)
    # Cached: subsequent calls return the same instance until cleared.
...
def test_verify_jwt_rejects_expired_token(
    ...
    """An expired token raises 401 with the documented detail."""
    token = make_token(
...
def test_verify_jwt_rejects_empty_string_sub(test_settings: Settings) -> None:
    """Empty-string `sub` is rejected (treated as missing)."""
    payload = {
        "sub": "",
        "email": "a@b.com",
        "jti": "j",
...
@pytest.mark.unit
def test_verify_jwt_rejects_empty_string_email(test_settings: Settings) -> None:
    """Empty-string `email` is rejected (treated as missing)."""
    payload = {
        "sub": "u",
        "email": "",
        "jti": "j",
...
def test_verify_jwt_rejects_empty_string_jti(test_settings: Settings) -> None:
    """Empty-string `jti` is rejected (treated as missing)."""
    payload = {
        "sub": "u",
        "email": "a@b.com",
        "jti": "",
...
@pytest.mark.unit
def test_verify_jwt_rejects_non_string_email(test_settings: Settings) -> None:
