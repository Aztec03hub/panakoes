...
| 5 | `infra/dev/secrets` | ~$0.40 | Secrets Manager secrets with placeholder values | $0.40/secret/month × 7 = ~$2.80. **You populate real values in Section D.** |
...
The `infra/dev/secrets` module creates 7 secret resources with **placeholder** strings (per `lifecycle { ignore_changes = [secret_string] }`). You write the real values via CLI; subsequent `terraform apply`s do not revert them.
