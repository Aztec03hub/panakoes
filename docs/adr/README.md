...
| ADR-043 | [AWS resource tagging strategy for cost allocation](ADR-043-aws-resource-tagging-strategy.md) | Every Panakoes AWS resource carries four mandatory tags (`Project`, `Environment`, `Service`, `Component`) via provider-level `default_tags` in each Terraform module under `infra/dev/`. Per-tenant runtime workloads add an optional `TenantId` tag. Activation in the Billing console is a separate operator step captured in `docs/runbooks/cost-allocation-tag-activation.md`. |
| ADR-044 | [Container Insights cost vs observability tradeoff in dev](ADR-044-container-insights-cost-tradeoff.md) | Disable ECS Container Insights in dev to drop the $44/mo line item (~30% of the post-Wave-1 dev bill); the health-aggregator falls back to ECS DescribeServices / DescribeTasks for the basic per-service view. Production re-enables once steady-state metric volume and observability needs are known. |
| ADR-045 | [File-defined long-running agents under `.claude/agents/`](ADR-045-file-defined-long-running-agents.md) | Recurring sub-agents (dependency updates, security sweeps, infra audits) are defined as Markdown files under `.claude/agents/`, versioned and PR-reviewed, dispatched by name. One-shot agents continue to use inline briefs from `docs/templates/agent-brief.md`. Both produce run reports in `.agent-runs/`. |
| ADR-046 | [Local-first verification as orchestrator discipline](ADR-046-local-first-verification-discipline.md) | Every agent brief MUST include a "Local-First Verification" section naming the exact commands, mandating FULL output capture in the run report, and defining a stop condition on failure. The orchestrator's trust-but-verify cycle is the second-line check; server-side CI is the third, not the first. |
