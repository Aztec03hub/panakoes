...
| `psql: command not found` inside container | Auth runtime image does not ship `psql` | Re-bake with `apk add postgresql-client` in the Dockerfile (or pivot the script to a Node-based shim) |
...
and the ECS exec session ends as soon as `psql` exits.
