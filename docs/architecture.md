...
| `summarization` | Py, FastAPI or Lambda | pending | Reads transcript from S3, calls Claude Haiku 4.5 (default) or Sonnet 4.6 (paid "deep summary" tier per ADR-013), writes Summary record. ECR repo + IAM role pre-provisioned. |
| `notification` | Py, Lambda | pending | Email (SES) and webhook fan-out on transcript-ready / billing / error events. ECR repo + IAM role pre-provisioned. |
| `session-manager` | Py, ECS Fargate or Lambda | pending | WebSocket session lifecycle: create, attach GPU, idle-timeout reap, graceful teardown. Persists state in `panakoes-dev-streaming-sessions`. |
...
| `panakoes-auth-client` | `services/auth-client-py/` (branch `feat/auth-client-py`) | Python-side JWT verifier. HS256 today, will swap to RS256+JWKS per ADR-022 in slice 2. | Every Python service that authenticates inbound HTTP. |
| `panakoes-models` | `services/models-lib/` | Shared Pydantic v2 models for cross-service contracts: `User`, `IngestionRecord`, `Transcript`, `Summary`, `StreamingSession`, `NotificationRecord`, `Subscription`, `ApiError`. `frozen=True`, `extra="forbid"`, UTC-only timestamps. | Every Python service that touches a domain object. |
| `panakoes-middleware` | `services/middleware-lib/` (branch `feat/middleware-lib`) | Cross-cutting FastAPI/Lambda middleware: request-id propagation, OTEL trace-id binding to log records, structured error envelope, audit-event helpers. | Every Python HTTP service. |
| `panakoes-otel` | `services/otel-lib/` | Single-call OTel bootstrap: `configure(service_name, environment)` wires TracerProvider + MeterProvider + LoggerProvider, OTLP/gRPC exporters, W3C TraceContext, resource attributes (`service.namespace=panakoes`). Auto-instrumentation helpers for FastAPI, boto3, httpx. | Every Python service. |
| `@panakoes/otel` | `services/otel-ts-lib/` (branch `feat/otel-ts-lib`) | TypeScript counterpart to `panakoes-otel`. Same resource convention, OTLP/gRPC export, auto-instrumentation for Hono / `@hono/node-server` / `pg`. | `auth` service and any future TS service. |
| `panakoes-test-helpers` | `services/test-helpers/` (branch `feat/test-infra-bundle`) | Shared pytest fixtures: testcontainers Postgres factory, moto-backed AWS service fixtures, JWT-mint helper, audit-store reset hook. | Every Python service's test suite. |
...
- **`dev/backup/`**. AWS Backup vault and plans for DDB tables and any future RDS instances. Cross-account copy not yet wired. **Plan-only** (branch `feat/dev-backup`).

### Not yet modeled

ECS cluster + Fargate services, Lambda functions, AWS Batch GPU compute environment + job queue + job definition, RDS or Aurora-Serverless-v2 for auth in prod, CloudFront distribution + ACM certificate, API Gateway REST + WebSocket, custom GPU AMI build via Packer. These land in subsequent slices; the IAM and ECR modules already forward-reference them via explicit ARN construction so policies stay tight from day zero.
